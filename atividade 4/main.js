...
function robotBodyVertices() {
    return new Float32Array(rectangleVertices(-0.1, -0.2, 0.2, 0.4));
}
...
function robotChestVertices() {
    return new Float32Array(rectangleVertices(-0.06, -0.1, 0.12, 0.2));
}
...
function robotMouthBackgroundVertices() {
    return new Float32Array(rectangleVertices(-0.04, 0.02, 0.08, 0.04));
}

function robotTeethVertices() {
    const vertices = [];
    vertices.push(...rectangleVertices(-0.035, 0.025, 0.015, 0.03));
    vertices.push(...rectangleVertices(-0.010, 0.025, 0.020, 0.03));
    vertices.push(...rectangleVertices( 0.020, 0.025, 0.015, 0.03));
...
class Robot {
    constructor(tx, ty, speed) {
        ...
        this.leftEye = new RobotPart(new Float32Array(circleVertices(0.015, 12)), amarelo); // <-- Olhos amarelos
        this.rightEye = new RobotPart(new Float32Array(circleVertices(0.015, 12)), amarelo); // <-- Olhos amarelos

        this.mouthBg = new RobotPart(robotMouthBackgroundVertices(), preto);
        this.teeth = new RobotPart(robotTeethVertices(), branco);

        this.leftArm = new RobotLimb(amareloEscuro, -0.12, 0.15); 
        this.rightArm = new RobotLimb(amarelo, 0.12, 0.15);     
        this.leftLeg = new RobotLimb(amareloEscuro, -0.06, -0.2); 
        this.rightLeg = new RobotLimb(amarelo, 0.06, -0.2);     
    }
    ...
    move() {
        ...
        this.time += 0.05; 
        const swingAngle = Math.sin(this.time) * 0.4; 
        ...
        // A boca e os dentes acompanham a matriz da cabeça
        ...
        const leftEyeTransform = m3.multiply(headTransform, m3.translation(-0.03, 0.1));
        this.leftEye.updateModelTransform(leftEyeTransform);
        
        const rightEyeTransform = m3.multiply(headTransform, m3.translation(0.03, 0.1));
